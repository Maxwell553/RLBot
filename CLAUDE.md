...
Library code lives in the `rlbot/` package (`data_utils.py`, `trading_env.py`, `rl_config.py`, `baselines.py`, `run_artifacts.py`, `inference_load.py`, `vecnorm_utils.py`, `modal_cloud.py`). CLIs live in `scripts/` (`train.py`, `backtest.py`, `modal_app.py`, `migrate_runs_layout.py`, `run_seed_ensemble.sh`). There is **no** top-level `train.py`/`backtest.py` and **no** `windows/` directory.
...
Gitignored (see `.gitignore`): `.cache/`, `data_cache.npz`, `Runs/`, the legacy artifact roots `models/ runs/ tb_logs/ logs/ plots/` (pre-`Runs/` layout, migratable via `scripts/migrate_runs_layout.py`), `ibkr_paper/`, and `execution/**` (except a tracked `execution/README.md`). Older docs/diagrams may mention `paper_trade/` or `ibkr_paper/`; those directories are not present in a fresh clone — do not assume they exist.
