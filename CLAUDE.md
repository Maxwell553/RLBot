...
- New env behavior knobs in `config.yaml`: `training.early_stop_patience` (default **8** — patience early-stop after the curriculum completes; `0` disables), `training.reproducible` (deterministic per-env seed streams), `reward.sortino_downside_floor` (downside-deviation floor in daily-return units, 0.001 = 10 bp/day; parser default 1e-4 preserves old run snapshots), and `data.feature_preroll_bars` (causal warmup window for `independent` split mode). Default training budget is **50M** steps (`training.timesteps` / `curriculum.budget_short`); fee/churn milestones use fractions **0.13** / **0.585** (~6.5M / ~29.25M).
