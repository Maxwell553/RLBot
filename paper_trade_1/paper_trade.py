#!/usr/bin/env python3
"""
Daily paper-trading pipeline for the 15M_4_15_26_h RL portfolio model.

Run once after market close:
    cd paper_trade_1 && python paper_trade.py
...
First run initialises a $10,000 cash portfolio. Each subsequent run:
  ...
  2. Builds the exact 88-dim observation the model was trained on
     (this model was trained WITHOUT MACD features)
  3. Runs the PPO policy to get target portfolio weights
  4. Simulates trade execution with realistic costs
  5. Persists state to state.json, appends to trade_journal.txt
"""
...
def fetch_recent(days: int = 90) -> Tuple[pd.DatetimeIndex, np.ndarray,
                                           np.ndarray, np.ndarray]:
    """Fetch recent daily data for all assets + macro, return aligned arrays.

    Returns (index, ohlcv, rsi, macro). MACD is not computed since
    model 15M_4_15_26_h was trained without it.
    """
    ...
    for ticker, yf_sym in {**YF_SYMBOLS, **MACRO_SYMBOLS}.items():
        obj = yf.Ticker(yf_sym)
        df = obj.history(start=since, interval="1d", auto_adjust=False)
        if df.empty:
    ...
    for ticker in TICKERS:
        ...
        merged[existing] = merged[existing].bfill()
    ...
    for j, ticker in enumerate(TICKERS):
        ...
        rsi_arr[:, j] = _rsi(close_s).bfill().fillna(50.0).to_numpy()
    ...
    return idx, ohlcv, rsi_arr, macro
...
def build_obs(
    ...
    """Build the 88-dim observation vector for 15M_4_15_26_h.
    ...
    for h in RETURN_HORIZONS:
        t0 = max(t_mkt - h, 0)
        rets = np.log((ohlcv[t_mkt, :, 3] + 1e-12) / (ohlcv[t0, :, 3] + 1e-12))
        parts.append(rets.astype(np.float32) * 100.0)
        parts.append(np.array([rets.mean()], dtype=np.float32) * 100.0)
    ...
    for h in RETURN_HORIZONS:
        t0 = max(t_mkt - h, 0)
        macro_rets = np.log((macro[t_mkt] + 1e-12) / (macro[t0] + 1e-12))
        parts.append(macro_rets.astype(np.float32) * 100.0)
...
def apply_holding_costs(cash: float, units: np.ndarray, close: np.ndarray) -> Tuple[float, float]:
    ...
    daily_costs = position_values * DAILY_HOLDING_COST
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Daily paper-trading pipeline (15M_4_15_26_h)")
    ...
    print(f"Paper Trade [15M_4_15_26_h] — {today}")
    print(f"{'(DRY RUN) ' if args.dry_run else ''}Fetching market data...")

    idx, ohlcv, rsi, macro = fetch_recent(days=90)
    ...
    obs = build_obs(ohlcv, rsi, macro, t, cash, units, peak_nav)
    assert obs.shape == (88,), f"Observation shape mismatch: {obs.shape}"

    obs_mean, obs_var = load_obs_normalizer()
    obs_normed = normalize_obs(obs, obs_mean, obs_var)

    from stable_baselines3 import PPO
    model = PPO.load(str(MODEL_ZIP), device="cpu")
    action, _ = model.predict(obs_normed.reshape(1, -1), deterministic=True)
    action = action.flatten()

    target_w = softmax(action.astype(np.float64))
    ...
    lines = []
    lines.append(f"{'=' * 60}")
    lines.append(f"=== [15M_4_15_26_h] {latest_date} (Day {day_number}) ===")
