...
Examples::
    ...
    python scripts/paper_core_equity.py reset
    python scripts/paper_core_equity.py run-day --refresh-data
    python scripts/paper_core_equity.py run-day --as-of 2026-08-21 --refresh-data
    python scripts/paper_core_equity.py run-day --dry-run
    python scripts/paper_core_equity.py activate

The headless 5m collector (``scripts/live_forward_loop.py`` / LaunchAgent)
MTM's lots and re-runs ``run-day`` once per session. After a cash reset it
retries until the book is filled so /ops/forward keeps moving like GE1.
"""
...
from rlbot.paper_core_equity import (  # noqa: E402
    PAPER_DIR,
    STATE_PATH,
    load_state,
    reset_paper_book,
...
from rlbot.pack_core_equity import PAPER_RUN_ID, STRATEGY_ID  # noqa: E402
...
def main() -> None:
    ...
    p_run.add_argument(
        "--activate",
        action="store_true",
        help="Point execution/forward_active.json at CORE_EQUITY (not the default)",
    )
    p_run.add_argument(
        "--no-activate",
        action="store_true",
        help="Kept for compatibility; CoreEquity paper never steals the GE1 forward pointer",
    )
    ...
    sub.add_parser("reset", help="Flat $100k cash book; skip a same-day refill (start next session)")
    sub.add_parser("activate", help="Point /ops/forward at CORE_EQUITY (existing mark)")

    args = parser.parse_args()

    if args.cmd == "reset":
        out = reset_paper_book()
        print(json.dumps(out, indent=2, default=str))
        print(
            f"[paper_ce] reset ${out['equity']:,.0f} cash; "
            f"no trades until after {out['hold_until']}"
        )
        return

    if args.cmd == "activate":
        set_active_forward_run(PAPER_RUN_ID)
        print(f"[paper_ce] active forward → {PAPER_RUN_ID}")
        return
    ...
    result = run_paper_day(
        ...
        set_active=bool(args.activate) and not bool(args.no_activate),
    ...
    if risky:
        print(f"[paper_ce] book (cash={tw.get('CASH', 0):.1%}):")
        for k, v in risky:
            print(f"  {k:8s} {float(v):6.2%}")
    print(
        f"[paper_ce] forward: /ops/forward (run_id={result.get('run_id')}) "
