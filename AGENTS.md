...
- **Reward shaping** (`config.yaml` → `reward`): return (downside amplification capped at `drawdown_amp_max: 4.0`; parser default 0 = uncapped preserves old snapshots) + benchmark excess + Sortino diff (capped together at the constant `benchmark_combined_abs_cap: 24.0`) + participation − inactivity (max **~0.30** at 100% cash; scaled down in stress via `inactivity_vix_relief: 1.0` — half at VIX 27, none at VIX ≥ 36; parser default 0 = no relief; `participation_vix_relief: 1.0` applies the same multiplier to the participation bonus, so shaping goes cash-vs-invested neutral in stress) − cost-linked churn − direct turnover penalty − direct drawdown penalty − concentration penalty − excess downside-volatility penalty − exposure risk penalty. **`max_single_asset_weight: 0.20`**. `cash_daily_yield` on idle cash (config default **0.00015**/day ≈ 3.8% ann.; parser default 0). **Fee/churn/turnover curriculum** on train **and** eval; **`models/best/`** after `fee_ramp_end` on **robust benchmark-relative eval score** (not raw mean NAV). In-training **`eval_portfolio_diagnostics.jsonl`**. See the reward table in `config/README.md`. OOS `backtest_summary.json` includes `portfolio_diagnostics` (exposure, diversification, turnover; cap-hit uses near-cap detection).
