...
- **Reward shaping** (`config.yaml` → `reward`): return + benchmark excess + Sortino diff (capped together at the constant `benchmark_combined_abs_cap: 24.0`) + participation − inactivity (max **~0.50** at 100% cash) − cost-linked churn − direct turnover penalty − direct drawdown penalty − concentration penalty − excess downside-volatility penalty − exposure risk penalty. **`max_single_asset_weight: 0.20`**. Optional `cash_daily_yield` on idle cash (default **0**). **Fee/churn/turnover curriculum** on train **and** eval; **`models/best/`** after `fee_ramp_end` on **robust benchmark-relative eval score** (not raw mean NAV). In-training **`eval_portfolio_diagnostics.jsonl`**. See README reward table. OOS `backtest_summary.json` includes `portfolio_diagnostics` (exposure, diversification, turnover; cap-hit uses near-cap detection).
