...
- **Reward shaping** (`config.yaml` → `reward`): return (downside amplification capped at `drawdown_amp_max: 4.0`; parser default 0 = uncapped preserves old snapshots) + benchmark excess + Sortino diff (capped together at the constant `benchmark_combined_abs_cap: 24.0`) − cost-linked churn − direct turnover penalty − direct drawdown penalty (level term coupled to gross via `drawdown_level_exposure_coupling: 0.5`) − downside-volatility penalty (`vol_penalty_mode: absolute`) − exposure risk penalty (`vix_positive`). **`max_single_asset_weight: 0.20`**. `cash_daily_yield` on idle cash (config default **0.0002**/day ≈ 5.0% ann.; parser default 0). Inactivity / participation / concentration knobs are **omitted** from the active config (parser default 0; still loadable from old run snapshots). **Fee/churn/turnover curriculum** on train **and** eval; **`models/best/`** after `fee_ramp_end` on **robust benchmark-relative eval score** (not raw mean NAV). In-training **`eval_portfolio_diagnostics.jsonl`**. See the reward table in `config/README.md`. OOS `backtest_summary.json` includes `portfolio_diagnostics` (exposure, diversification, turnover; cap-hit uses near-cap detection).
