...
This file provides guidance to coding agents (e.g. OpenAI Codex, Claude, Gemini, etc) when working with code in this repository. It mirrors `CLAUDE.md`.

MarketTrainer (RLBot) trains a RecurrentPPO (LSTM) agent on a multi-asset daily portfolio environment, with strict chronological out-of-sample holdouts. `README.md` is the canonical reference for the asset universe, observation layout, reward formula, and walk-forward windows — read it for those details. This file covers commands and the invariants that span files. The **current published walk-forward OOS cohorts** are in [docs/RESEARCH.md](docs/RESEARCH.md#current-published-cohorts).

Library code lives in the `rlbot/` package (`data_utils.py`, `trading_env.py`, `rl_config.py`, `baselines.py`, `run_artifacts.py`, `run_audit.py`, `workflow_store.py`, `cohort_labels.py`, `curriculum_preflight.py`, `checkpoint_selection.py`, `two_head_actions.py`, `inference_load.py`, `inference_output.py`, `vecnorm_utils.py`, `modal_cloud.py`, `stats.py`, `reward_logging.py`, `visualize.py`, `research/`). CLIs live in `scripts/` (`train.py`, `backtest.py`, `modal_app.py`, `infer_weights.py`, `research.py`, `audit_runs.py`, `preflight.py`, `dashboard.py`, `frontend_api.py`, `workflow_api.py`, `shadow_trade.py`, `migrate_runs_layout.py`, `run_seed_ensemble.sh`). There is **no** top-level `train.py`/`backtest.py`, **no** `windows/` directory, and **no** `paper_trade/` tree.
...
# Run audit / curriculum preflight / local dashboard (no OOS reads)
python scripts/audit_runs.py --write
python scripts/preflight.py --config config/config.yaml
python scripts/dashboard.py

# Frontend services: read-only operator artifacts and separate mandate workflow
...
- Each training run snapshots the active config into `Runs/<run_id>/config.yaml` and writes `Runs/<run_id>/manifest.json` for reproducibility. Treat config + run_id as a pair. `scripts/backtest.py` and `scripts/infer_weights.py` load that snapshot by default (see "Train/backtest must agree"). Frozen cohort recipes may live under `config/cohorts/` or be copied from `Runs/<run_id>/config.yaml`; pass them with `--config`.

`config.yaml` lists per-asset arrays (slippage, tx_fee, holding cost, benchmark cap weights) that **must have exactly N entries** in `universe.assets` key order, where `N` = number of `universe.assets` keys (default 10: SP500, GOLD, OIL, EURUSD, USDJPY, NIKKEI, FTSE, BOND10Y, COPPER, EM; supported range 5–55). `rl_config._float_list` / `validate_config_for_universe` enforce the length match; `--n-assets N` slices the first N keys and renormalizes benchmark weights. `universe.benchmark` is a reporting sleeve for benchmark-only/SPY and 60/40 diagnostics; reward benchmark excess and Sortino use `reward.benchmark_cap_weights`, while best-model eval selection uses `training.best_model_benchmark` (default `equal_weight_daily`).
...
- `--checkpoint` defaults to **`best`** (robust **benchmark-relative** eval-score-selected **after `fee_ramp_end`**: `(1−blend)×mean(excess) + blend×stitched_excess − std_coef×std(excess) − dd_coef×p75(max_dd_nav)` vs **equal-weight daily**; with `training.best_model_score_mode: excess_sharpe` — the current config default — the excess signal is the **annualized Sharpe of daily excess returns** (segment-mean blended with pooled) and the dd penalty is the unitless `p75(max_dd_frac)`; `excess_nav` is the legacy NAV-dollar mode preserved for old snapshots; holdout not used to pick weights); `latest`/`both` print an OOS-touch warning. The summary's `checkpoint_label` is derived from the weights actually evaluated (best/final/latest), never from `--plot-tag`. **`best_model.zip` is saved with `best/vec_normalize.pkl` from the same eval step** — never pair best weights with end-of-run normalization stats.
...
`N_ACTIONS = N + 1` (cash + N risky assets); for the default N=10 that is 11 actions. The policy outputs `Box(-3, 3)^(N+1)` → optional EMA smoothing on the logits → softmax (cash competes) → live-mask → per-risky-asset clip-and-redistribute cap (`max_single_asset_weight`, default **0.20**) → long-only simplex (`portfolio_weights_from_action`). Observation dimension is `observation_dim_for_universe(N)`: base `10*N + 28` (128 for N=10) plus 4 portfolio self-state features (realized vol, downside vol, rolling excess vs benchmark, near-cap fraction) when `environment.self_state_features` is on — the config default enables it, giving **132** for N=10; old run snapshots without the key keep their original layout. Macro series (DXY, TNX, VIX, HY OAS; 4 series) feed the observation **only** — they are not tradeable. `N` is dynamic (5–55); derive dims from `N`, never hard-code action/observation dimensions (e.g. 11, 118).
...
- **Reward shaping** (`config.yaml` → `reward`): return (downside amplification capped at `drawdown_amp_max: 4.0`; parser default 0 = uncapped preserves old snapshots) + benchmark excess + Sortino diff (capped together at the constant `benchmark_combined_abs_cap: 24.0`) + participation − inactivity (max **~0.20** at 100% cash in calm non-DD regimes; stress-relieved by most-relief-wins of `inactivity_vix_relief` and `inactivity_drawdown_relief` — cash is cheaper in VIX stress *or* when the agent is already underwater; same combine on participation via `participation_*_relief`) − cost-linked churn − direct turnover penalty − direct drawdown penalty (level term coupled to gross via `drawdown_level_exposure_coupling: 1.0` so de-risking to cash zeros the persistent level tax; ~10/step at 15% DD full gross vs ~0.20 max inactivity) − concentration penalty − excess downside-volatility penalty − exposure risk penalty. **`max_single_asset_weight: 0.20`**. `cash_daily_yield` on idle cash (config default **0.00015**/day ≈ 3.8% ann.; parser default 0). **Fee/churn/turnover curriculum** on train **and** eval; **`models/best/`** after `fee_ramp_end` on **robust benchmark-relative eval score** (not raw mean NAV). In-training **`eval_portfolio_diagnostics.jsonl`**. See the reward table in `config/README.md`. OOS `backtest_summary.json` includes `portfolio_diagnostics` (exposure, diversification, turnover; cap-hit uses near-cap detection).
...
- New env behavior knobs in `config.yaml`: `training.early_stop_patience` (default **12** — patience early-stop after the curriculum completes; `0` disables), `training.reproducible` (deterministic per-env seed streams), `training.eval_freq_steps` / `training.eval_freq_pre_gate_steps` (post/pre fee-ramp eval cadence), `reward.sortino_downside_floor` (downside-deviation floor in daily-return units, 0.001 = 10 bp/day; parser default 1e-4 preserves old run snapshots), `reward.vol_penalty_scale` (penalizes excess downside vol versus the passive benchmark over the Sortino window), `reward.inactivity_vix_relief` / `reward.participation_vix_relief` / `reward.inactivity_drawdown_relief` / `reward.participation_drawdown_relief` / `reward.drawdown_level_exposure_coupling` / `reward.drawdown_amp_max` / `reward.cash_daily_yield` (regime-aware risk-off shaping + reward outlier cap; parser defaults 0 preserve old snapshots), `environment.self_state_features` (+4 obs features; parser default false), `training.best_model_score_mode` (`excess_sharpe` current default; parser default `excess_nav`), and `data.feature_preroll_bars` (causal warmup window for `independent` split mode). Default training budget is **50M** steps (`training.timesteps` / `curriculum.budget_short`); fee/churn milestones use fractions **0.10** / **0.45** (~5M / ~22.5M), DR widening ends at ~35M (`dr_widen_span_fraction: 0.25`), leaving ~15M stationary full-DR. `hyperparameters.lr_schedule: phase_aware` (current default; parser default `cosine` preserves old snapshots) holds the initial LR until DR widening completes, then cosine-decays to the floor. Checkpoint selection uses a trailing-3 median of post-gate scores blended 0.3 with a fixed 1.5×-fee/lag-2 stress eval (`training.best_model_trailing_evals: 3`, `best_model_stress_suite: true`; benchmark priced at the same stress fees).
...
Gitignored (see `.gitignore`): `.cache/`, `data_cache.npz`, `Runs/`, `paper/`, and `execution/**` (reserved for the tier-5 shadow ledger / future broker state; nothing under `execution/` is tracked and the directory may not exist). The legacy artifact roots (`models/ runs/ tb_logs/ logs/ plots/`, pre-`Runs/` layout) are no longer ignore-listed — migrate them via `scripts/migrate_runs_layout.py` rather than leaving them in the tree. Broker automation and local execution state stay out of the research tree.
