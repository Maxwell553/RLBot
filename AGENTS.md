...
MarketTrainer (RLBot) trains a RecurrentPPO (LSTM) agent on a multi-asset daily portfolio environment, with strict chronological out-of-sample holdouts. `README.md` is the canonical reference for the asset universe, observation layout, reward formula, and walk-forward windows — read it for those details. This file covers commands and the invariants that span files.

Library code lives in the `rlbot/` package (`data_utils.py`, `trading_env.py`, `rl_config.py`, `baselines.py`, `run_artifacts.py`, `inference_load.py`, `vecnorm_utils.py`, `modal_cloud.py`, and `research/` for the auto-research spec/gates/registry/report). CLIs live in `scripts/` (`train.py`, `backtest.py`, `research.py`, `infer_weights.py`, `paper_trade.py`, `modal_app.py`, `migrate_runs_layout.py`, `run_seed_ensemble.sh`). There is **no** top-level `train.py`/`backtest.py` and **no** `windows/` directory.
...
# Train (defaults from config/config.yaml; first run needs --refresh-data to fetch yfinance data)
python scripts/train.py --refresh-data
python scripts/train.py --config path/to/config.yaml   # override hyperparameters
python scripts/train.py --window 1 --run-id my_run     # walk-forward window via date flags
...
python scripts/backtest.py --run-ids W1,W2,W3 --checkpoint best          # batch, one process
...
modal run scripts/modal_app.py -- --modal-gpu H100 --window 1 --run-id my_run
...
- Each training run snapshots `config/config.yaml` into `Runs/<run_id>/config.yaml` and writes `Runs/<run_id>/manifest.json` for reproducibility. Treat config + run_id as a pair. (Caveat: `scripts/backtest.py` does **not** auto-load that snapshot — see "Train/backtest must agree".)
...
- `backtest.py` loads `Runs/<run_id>/config.yaml` for costs/cap/env mechanics (override with `--use-current-config`) and prefers the run-local `Runs/<run_id>/data_cache.npz` over the global cache (override with `--data-cache PATH`; a missing snapshot triggers a loud reproducibility warning), so editing config or refreshing the cache does not silently change an old run's OOS numbers.
- `--checkpoint` defaults to **`best`** (eval-NAV-selected; holdout not used to pick weights); `latest`/`both` print an OOS-touch warning. The summary's `checkpoint_label` is derived from the weights actually evaluated (best/final/latest), never from `--plot-tag`.
- Each backtest writes `Runs/<run_id>/backtest_summary.json` (override `--summary-json`) with metrics plus config/data-cache hashes, and **compares** those hashes to the training manifest (mismatch → `hash_drift` field + warning).
- A missing `vec_normalize.pkl` on a `norm_obs: true` run is a **hard error** (raw-obs metrics are meaningless); `--allow-raw-obs` overrides explicitly. Same guard in `scripts/infer_weights.py`.
- `best_model.zip` is saved together with the VecNormalize stats it was selected under (`EvalNavBestModelCallback` writes `models/best/vec_normalize.pkl` at best-save time; the end-of-training copy is only a fallback).
- The final manifest write **updates** the pre-training manifest (preserving `chronological_holdout`) and stamps `training_status: completed|interrupted`.

## Inference freezes normalization

`VecNormalize` updates running obs/reward statistics during training. For any out-of-sample rollout (`scripts/backtest.py`, paper trading), freeze it first via `rlbot.vecnorm_utils.freeze_vec_normalize_for_inference()` — it sets `training=False`, disables reward norm, keeps obs norm. `rlbot/inference_load.py` (`load_recurrent_ppo_inference`, `load_vec_normalize_for_inference`) wraps this for backtest. Never let OOS data update the running stats.
...
- **`scripts/infer_weights.py`** emits audited target weights (long-only simplex ≤ cap) with full provenance (config/data/model hashes), reusing the backtest rollout. `scripts/paper_trade.py` + `paper_trade/` are measurement only — no broker, no market-impact/capacity model.
...
- `train.py` refuses an existing `Runs/<run_id>/` (manifest present) unless `--overwrite-run` (which also clears the stale best-eval threshold and best-model artifacts) or `--resume` is passed.
...
Gitignored (see `.gitignore`): `.cache/`, `data_cache.npz`, `Runs/`, the legacy artifact roots `models/ runs/ tb_logs/ logs/ plots/` (pre-`Runs/` layout, migratable via `scripts/migrate_runs_layout.py`), `ibkr_paper/`, and `execution/**` (reserved; nothing under `execution/` is tracked and the directory may not exist). `paper_trade/README.md` and `scripts/paper_trade.py` **are** tracked (measurement-only, no broker); `ibkr_paper/` is not present in a fresh clone — do not assume it exists.
